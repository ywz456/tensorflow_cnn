...
#     return id_list
#将少量样本强行复制6份
def no_class(image_dir,labels):
    id_list = []
    image_basenames = os.listdir(image_dir)
    for image_basename in image_basenames:
        label = image_basename.split('.')[0]
        if labels[int(label)]==1:
            id_list.append(image_basename.split('.')[0])
            id_list.append(image_basename.split('.')[0])
            id_list.append(image_basename.split('.')[0])
            id_list.append(image_basename.split('.')[0])
...
id_list = no_class(image_dir,labels)
...
with tf.name_scope('final_training_ops'):
    ...
    final_tensor = tf.nn.dropout(final_tensor, keep_prob)
...
with tf.Session() as sess:
    ...
    STEPS = 4000  # 迭代次数暂时设为4000
    for i in range(STEPS):
        # train_bottlenecks,train_labels = get_random_train_bottlenecks(train_id_list,labels,BATCH)
        train_bottlenecks,train_labels = get_random_train_bottlenecks(id_list,labels,BATCH)
        sess.run(train_step, feed_dict={bottleneck_input: train_bottlenecks, label_input: train_labels,keep_prob:0.5})

        if i % 100 == 0 or i + 1 == STEPS:
            # train_bottlenecks,train_labels = get_random_train_bottlenecks(train_id_list,labels,BATCH)
            train_bottlenecks, train_labels = get_random_train_bottlenecks(id_list, labels, BATCH)
            train_accuracy = sess.run(evaluation_step, feed_dict={bottleneck_input: train_bottlenecks, label_input: train_labels,keep_prob:1.0})
            print('经过%d轮训练，正确率为%.1f%%'%(i,train_accuracy*100))
